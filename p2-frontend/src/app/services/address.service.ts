...
export class AddressService {
  ...
  setCurrentAddress(address: Address, id: any){
    console.log("setting current address: ", address);
    console.log("this should be the address number: ", address.number);
    this.currentAddress = new currentAddressToEdit(address.addressId, address.number, address.street, address.city, address.state, address.zipCode, id);
    console.log("this is now the set address: ", this.currentAddress);
  }
  getCurrentAddress(){
  ...
  //PUT
  updateAddress(body: object, options: object): Observable<any> {
    console.log("PUT: ", this.baseUrl + "/update", body, options);
  ...
  //DELETE
  deleteAddress(uri: string, address: Address, options: object): Observable<any> {

    console.log("DELETE: ", this.baseUrl + uri, address.userId, options);
    //this return request is what gets sent to back end with the url same as on postman to update info to the backend.
    return this.http.delete<any>(this.baseUrl + uri , options)
      .pipe(
        retry(1),
        catchError(this.errorHandler)
      )
...
export class currentAddressToEdit {
  addressId: any;
  number: string = "";
  street: string = "";
  city: string = "";
  ...
  constructor(_addressId: any, _number: string, _street: string, _city: string, _state: string, _zipCode: string, _userId: any) {
    //id needs to be the address id same as the last address the user input. 
    //I need a variable that keeps track of the last id that's generated by user.

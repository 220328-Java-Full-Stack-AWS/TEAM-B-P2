...
export class AddressService {
  ...
  getAddressById(uri: string, body: object, options: object): Observable<any> {
    ...
    return this.http.get<any>(this.baseUrl + uri + JSON.stringify(body), this.httpOptions)
      .pipe(
        retry(1),
        catchError(this.errorHandler)
      )
  ...
  getAllAddress(): Observable<any> {
    console.log("View all Address");
    return this.http.get<object>(this.baseUrl + "/address/all", { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) })
      .pipe(
        retry(1),
        catchError(this.errorHandler)
      )
  ...
  updateAddress(uri: string, body: object, options: object): Observable<any> {
    ...
    return this.http.put<any>(this.baseUrl + uri, JSON.stringify(body), this.httpOptions)
      .pipe(
        retry(1),
        catchError(this.errorHandler)
      )
...
export class Address {
  ...
  constructor(_number: string, _street: string, _city: string, _state: string, _zipCode: string) {
